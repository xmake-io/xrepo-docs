...
```console
xrepo install -p wasm approval-tests-cpp
```
...
```console
xrepo install -p wasm astc-encoder
```
...
```console
xrepo install -p wasm aui-toolbox
```
...
```console
xrepo install -p wasm base64-zhicheng
```
...
```console
xrepo install -p wasm beaengine
```
...
```console
xrepo install -p wasm boost_math
```
...
```console
xrepo install -p wasm boringssl
```
...
```console
xrepo install -p wasm byte-lite
```
...
```console
xrepo install -p wasm coin-or-coinutils
```
...
```console
xrepo install -p wasm coin-or-osi
```
...
```console
xrepo install -p wasm console-bridge
```
...
```console
xrepo install -p wasm cwt-cucumber
```
...
### emscripten-glfw (wasm)


| Description | *This project is an emscripten port of GLFW written in C++ for the web/webassembly platform #wasm* |
| -- | -- |
| Homepage | [https://pongasoft.github.io/emscripten-glfw/test/demo/main.html](https://pongasoft.github.io/emscripten-glfw/test/demo/main.html) |
| License | Apache-2.0 |
| Versions | 2024.09.07 |
| Architectures | wasm32, wasm64 |
| Definition | [emscripten-glfw/xmake.lua](https://github.com/xmake-io/xmake-repo/blob/master/packages/e/emscripten-glfw/xmake.lua) |

##### Install command

```console
xrepo install -p wasm emscripten-glfw
```

##### Integration in the project (xmake.lua)

```lua
add_requires("emscripten-glfw")
```
...
```console
xrepo install -p wasm esp32-arduino-libs
```
...
```console
xrepo install -p wasm esp32-devel
```
...
```console
xrepo install -p wasm extra-cmake-modules
```
...
```console
xrepo install -p wasm fast-lzma2
```
...
```console
xrepo install -p wasm flatbuffers
```
...
```console
xrepo install -p wasm fontconfig
```
...
```console
xrepo install -p wasm gelldur-eventbus
```
...
```console
xrepo install -p wasm geo-utils-cpp
```
...
```console
xrepo install -p wasm geode-sdk-result
```
...
```console
xrepo install -p wasm gfx-timsort
```
...
```console
xrepo install -p wasm google-cloud-cpp
```
...
```console
xrepo install -p wasm hiredis-vip
```
...
```console
xrepo install -p wasm hyperscan
```
...
```console
xrepo install -p wasm hyprwayland-scanner
```
...
```console
xrepo install -p wasm imgui-file-dialog
```
...
```console
xrepo install -p wasm jerryscript
```
...
```console
xrepo install -p wasm json-schema-validator
```
...
```console
xrepo install -p wasm khaledhosny-ots
```
...
```console
xrepo install -p wasm kotlin-native
```
...
| License | MPL-2.0 |
| Versions | v0.23.1, v0.23.2, v0.24.1, v0.24.2, v0.24.3, v0.24.4, v0.24.5 |
| Architectures | wasm32, wasm64 |
...
```console
xrepo install -p wasm libdatachannel
```
...
```console
xrepo install -p wasm libdispatch
```
...
```console
xrepo install -p wasm libid3tag
```
...
```console
xrepo install -p wasm libkdtree
```
...
```console
xrepo install -p wasm libkeyfinder
```
...
```console
xrepo install -p wasm libmysofa
```
...
```console
xrepo install -p wasm libnpy-matajoh
```
...
```console
xrepo install -p wasm libnyquist
```
...
```console
xrepo install -p wasm libopenmpt
```
...
```console
xrepo install -p wasm librttopo
```
...
```console
xrepo install -p wasm libsdl2_image
```
...
```console
xrepo install -p wasm libsdl2_mixer
```
...
```console
xrepo install -p wasm libsdl2_mixer_x
```
...
```console
xrepo install -p wasm libsdl2_net
```
...
```console
xrepo install -p wasm libsdl2_ttf
```
...
```console
xrepo install -p wasm libsdl3_image
```
...
```console
xrepo install -p wasm libsdl3_ttf
```
...
```console
xrepo install -p wasm libzchunk
```
...
```console
xrepo install -p wasm liquid-dsp
```
...
```console
xrepo install -p wasm llama.cpp
```
...
```console
xrepo install -p wasm log4cplus
```
...
```console
xrepo install -p wasm macdylibbundler
```
...
```console
xrepo install -p wasm marchingcubecpp
```
...
```console
xrepo install -p wasm meshoptimizer
```
...
```console
xrepo install -p wasm microsoft-kuku
```
...
```console
xrepo install -p wasm minizip-ng
```
...
```console
xrepo install -p wasm msdf-atlas-gen
```
...
```console
xrepo install -p wasm muparserx
```
...
```console
xrepo install -p wasm nanoarrow
```
...
```console
xrepo install -p wasm nazarautils
```
...
```console
xrepo install -p wasm node-addon-api
```
...
```console
xrepo install -p wasm node-api-stub
```
...
```console
xrepo install -p wasm nv-codec-headers
```
...
```console
xrepo install -p wasm obfusheader.h
```
...
```console
xrepo install -p wasm observer-ptr-lite
```
...
```console
xrepo install -p wasm offsetallocator
```
...
```console
xrepo install -p wasm opencolorio
```
...
```console
xrepo install -p wasm opensubdiv
```
...
```console
xrepo install -p wasm orocos-kdl
```
...
```console
xrepo install -p wasm paul_thread_pool
```
...
```console
xrepo install -p wasm pinocchio
```
...
```console
xrepo install -p wasm protobuf-cpp
```
...
```console
xrepo install -p wasm qt6network
```
...
```console
xrepo install -p wasm qt6widgets
```
...
```console
xrepo install -p wasm riscv32-esp-elf
```
...
```console
xrepo install -p wasm simplecpp
```
...
```console
xrepo install -p wasm sokol-tools
```
...
```console
xrepo install -p wasm spirv-cross
```
...
```console
xrepo install -p wasm sqlite-vec
```
...
| License | Public Domain |
| Versions | 3.23.0+0, 3.24.0+0, 3.34.0+100, 3.35.0+300, 3.35.0+400, 3.36.0+0, 3.37.0+200, 3.39.0+200, 3.43.0+200, 3.45.0+200, 3.45.0+300, 3.45.0+100, 3.46.0+0, 3.46.0+100, 3.47.0+100, 3.47.0+200, 3.47.0+0, 3.48.0+0, 3.49.0+0, 3.49.0+100, 3.49.0+200, 3.50.0+0, 3.50.0+100, 3.50.0+200, 3.50.0+300, 3.50.0+400, 3.51.0+100, 3.51.0+0, 3.51.0+300, 3.51.0+200, 3.53.0+100, 3.53.0+200, 3.53.0+0, 3.53.0+400 |
| Architectures | wasm32, wasm64 |
...
```console
xrepo install -p wasm taocpp-json
```
...
```console
xrepo install -p wasm tiny-aes-c
```
...
```console
xrepo install -p wasm tinycthread
```
...
```console
xrepo install -p wasm tinyobjloader
```
...
```console
xrepo install -p wasm tinyspline
```
...
```console
xrepo install -p wasm trianglemeshdistance
```
...
```console
xrepo install -p wasm verilator
```
...
### vladimirshaleev-ipaddress (wasm)
...
```console
xrepo install -p wasm vladimirshaleev-ipaddress
```
...
```console
xrepo install -p wasm wgsl-validator
```
...
```console
xrepo install -p wasm xor_singleheader
```
...
```console
xrepo install -p wasm xproperty
```
...
```console
xrepo install -p wasm xz-embedded
```
...
```console
xrepo install -p wasm yaml_cpp_struct
```
